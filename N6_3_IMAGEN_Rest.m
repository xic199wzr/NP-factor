 ...
 FU2_rest_sub      = table2array(TableS9_2.FU2_Rest_tale(:,1));
  ...
  %% BL Task
 [~,ind1,ind2] = intersect(FU2_Rest_tale.subject,BL_FC_table.subject);
 
 FU2_rest = table2array(FU2_Rest_tale(ind1,2:end));
 BL_Task  = table2array(BL_FC_table(ind2,2:end));
 
 nan = sum(isnan(FU2_rest),2);
 
 
 [r p] = corr(FU2_rest(nan==0,1),BL_Task(nan==0,2))
 
  
 [r p] = corr(FU2_rest(nan==0,1),BL_Task(nan==0,3))
 
  [r p] = corr(BL_Task(nan==0,:));
  
  [r p] = corr(FU2_rest(nan==0,:));
  
  plot(FU2_rest(nan==0,1),FU2_rest(nan==0,2),'.')
