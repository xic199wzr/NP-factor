...
load('/share/inspurStorage/home1/ISTBI_data/ABCD_V3/ABCD_SST/Conn/ABCD_SST_CONN_v3.mat')

across = TableS7.TableS7_1_Across;
name = {'SST_success'};
nums= [3 4];

sst_Data{1}  = reshape(ABCD_SST_StopFailure,[],size(ABCD_SST_StopFailure,3));
  ...
  Sub_SST_id = strrep(Sub_SST,'sub-NDAR','NDAR_');
 ...
 ABCD_FC_SST = table;
 ABCD_FC_SST.subID = Sub_SST_id;
 
  ABCD_FC_SST.ex_pp      = sum(cell2mat(Across_FC.ex_pp_only_FC_sum'),2);
  ABCD_FC_SST.ex_nn      = sum(cell2mat(Across_FC.ex_nn_only_FC_sum'),2);
  ABCD_FC_SST.in_pp       = sum(cell2mat(Across_FC.in_pp_only_FC_sum'),2);
  ABCD_FC_SST.in_nn       = sum(cell2mat(Across_FC.in_nn_only_FC_sum'),2);
  ABCD_FC_SST.oppo_pp  = sum(cell2mat(Across_FC.oppo_pp_FC_sum'),2);
  ABCD_FC_SST.oppo_pn  = sum(cell2mat(Across_FC.oppo_pn_FC_sum'),2);
  ABCD_FC_SST.oppo_np  = sum(cell2mat(Across_FC.oppo_np_FC_sum'),2);
  ABCD_FC_SST.oppo_nn  = sum(cell2mat(Across_FC.oppo_nn_FC_sum'),2);

save N5_3_2_2_ABCD_FC_SST_v2.mat ABCD_FC_SST Across_FC_mat
