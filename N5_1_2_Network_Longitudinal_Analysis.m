...
% [~,ind1,ind2] = intersect(BL_subject_residual,BL_FC_table.subject);
% [r p] = corr(table2array(BL_FC_table(ind1,2:end)),BL_residual(ind2,:))
...
[r p] = partialcorr(FU2_rest,FU2_exter,FU2_task,'row','complete')
[r p] = corr(FU2_rest,FU2_exter,'row','complete')

[r p] = corr(FU2_task,FU2_anxiety,'row','complete')
[r p] = partialcorr(FU2_task,FU2_exter,FU2_rest,'row','complete')

[bb,dev,stats] = glmfit([FU2_task,FU2_rest],FU2_anxiety);
anova(stats)
...
% figure
label_name = strrep(BL_FC_table.Properties.VariableNames(2:end),'_','-');
FC = [mean(BL_FC);mean(FU2_FC)]';
bar(FC)
xticklabels(label_name)
xtickangle(30)
legend({'BL-FC','FU2-FC'})
ylabel('Sum FC strength')

save A5_1_1_3_Network_Longitudinal_Analysis TableS9_4
