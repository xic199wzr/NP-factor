...
[r p t n] = xic_corr(BL_all_beha(ind5,2),BL_all_beha(ind5,5))
[r p t n] = xic_corr(mean(predict_anxiety,2),mean(predict_adhd,2))


% Module
pos_feed = cell2mat(Predict_pos(:,6)') + cell2mat(Predict_neg(:,6)');
pos_feed = mean(pos_feed(ind2,Both_filt(:,5)),2);
pos_feed_behav = mean(BL_all_beha(ind5,Both_filt(:,5)),2);
[r p t n] = xic_corr(pos_feed,pos_feed_behav)


Emotion_predict  = xic_predict_beha(Predict_pos,Predict_neg,Both_filt,[7,8]);
Emotion_angry_beha = xic_behav_residual(BL_all_beha,Both_filt,8);
[r p t n] = xic_corr(Emotion_predict(ind3),Emotion_angry_beha(ind5))

SST_predict  = xic_predict_beha(Predict_pos,Predict_neg,Both_filt,[1,2,3]);
SST_predict_beha = xic_behav_residual(BL_all_beha,Both_filt,[1 2 3 4 5 7 8]);
[r p t n] = xic_corr(SST_predict(ind1),SST_predict_beha(ind5))

MID_predict  = xic_predict_beha(Predict_pos,Predict_neg,Both_filt,[4,5,6]);
MID_predict_beha = xic_behav_residual(BL_all_beha,Both_filt,[1:8]);
[r p t n] = xic_corr(MID_predict(ind2),MID_predict_beha(ind5))

[r p] = corr(SST_predict(ind1),Emotion_predict(ind3))
