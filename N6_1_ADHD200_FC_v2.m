...
bar([mean(FC_mats(control,5)),mean(FC_mats(ADHD,5)),mean(FC_mats(exter,5)),mean(FC_mats(inter,5))]);

fc_se(1) = xic_se(FC_mats(control,5));
fc_se(2) = xic_se(FC_mats(ADHD,5));
fc_se(3) = xic_se(FC_mats(exter,5));
fc_se(4) = xic_se(FC_mats(inter,5));

fc_mean(1) = mean(FC_mats(control,5));
fc_mean(2) = mean(FC_mats(ADHD,5));
fc_mean(3) = mean(FC_mats(exter,5));
fc_mean(4) = mean(FC_mats(inter,5));


figure(1)
bar(fc_mean); hold on
errorbar(fc_mean,fc_se)
xlim([0 5]);
ylim([0 100]); 
xticklabels({'Control','ADHD','ADHD-exter','ADHD-internal'})
